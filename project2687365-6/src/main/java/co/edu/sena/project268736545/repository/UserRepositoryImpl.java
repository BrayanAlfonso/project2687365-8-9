...
public class UserRepositoryImpl implements Repository<co.edu.sena.project2687365.model.User>{
    ...
    public Integer saveObj(User user) throws SQLException {
        ...
        if (user.getUser_id() != null && user.getUser_id() > 0){
            sql = "update users_tbl u set u.user_firstname = ?, u.user_lastname = ?, u.user_email = ?, "+
                    "user_password = aes_encrypt (?,'$2a$12$hC77LspyLSLs9L1EGO2G3ODPNocQhIjgWlNznT//vBOz85YaOQbJu')"+
            "where user_id = ?";
        }else{
            sql = "insert into users_tbl (user_firstname, user_lastname, user_email, user_password)"+
                   "values(upper(?), upper(?), lower(?), "+
                    "aes_encrypt(?,'$2a$12$hC77LspyLSLs9L1EGO2G3ODPNocQhIjgWlNznT//vBOz85YaOQbJu'))";
        }
